...
export class Signup {
  ...
  onSubmit(): void {
    ...
    this.userService.checkEmailExists(email).subscribe({
      next: (users) => {
        if (users.length > 0) {
          this.message      = { type: 'error', text: 'An account with this email already exists.' };
        ...
        this.userService.register(name, email, password).subscribe({
          ...
          error: () => {
            this.message      = { type: 'error', text: 'Could not create account. Please try again.' };
            this.isSubmitting = false;
          },
        });
      },
      error: () => {
        this.message      = { type: 'error', text: 'Server error. Is json-server running?' };
        this.isSubmitting = false;
      },
    });
  }
